...
@Component({
  selector: 'app-agsousse',
  templateUrl: './agsousse.component.html',
  styleUrl: './agsousse.component.css'
})
export class AgsousseComponent {
  employes: any[] = [];
  ...
  hasMatchingEmploye(agence: string): boolean {
    return this.employes.some(
      employe => employe.agence == agence && employe.direction == 'sousse' && employe.siege == false
    );
  }
