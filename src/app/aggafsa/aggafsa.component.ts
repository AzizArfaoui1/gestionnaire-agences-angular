...
@Component({
  selector: 'app-aggafsa',
  templateUrl: './aggafsa.component.html',
  styleUrl: './aggafsa.component.css'
})
export class AggafsaComponent {
  employes: any[] = [];
  ...
  hasMatchingEmploye(agence: string): boolean {
    return this.employes.some(
      employe => employe.agence == agence && employe.direction == 'gafsa' && employe.siege == false
    );
  }
